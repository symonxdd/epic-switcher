...
export const EmojiReactions = () => {
  ...
  useEffect(() => {
    ...
    const fetchCounts = async () => {
      try {
        ...
        // Best Practice: Detect if the API endpoint actually exists and returns JSON
  ...
  return (
    <motion.div
      ...
      className="fixed bottom-8 left-8 z-[100] flex flex-col items-center gap-2"
    >
      <div className="px-3 py-1 bg-white/40 dark:bg-black/20 backdrop-blur-md border border-black/5 dark:border-white/10 rounded-full shadow-sm">
        <span className="text-xs font-semibold text-black/70 dark:text-foreground/80 tracking-tight">
        ...
        {REACTION_EMOJIS.map((emoji) => {
          ...
          return (
            <button
              ...
              className={`group relative flex items-center gap-2 px-3 py-2 rounded-full transition-all duration-300 ${hasReacted
                ? 'bg-primary/10 dark:bg-white/10 cursor-default'
                : 'hover:bg-black/5 dark:hover:bg-white/5 active:scale-90'
                }`}
            >
              <span className={`text-xl transition-transform duration-300 ${!hasReacted && apiAvailable && 'group-hover:scale-125'}`}>
                {emoji.char}
              </span>
              ...
              <AnimatePresence mode="wait">
                {count > 0 && (
                  <motion.span
                    ...
                    className={`text-xs font-bold font-mono ${hasReacted ? 'text-primary dark:text-white' : 'text-muted-foreground'
                      }`}
                  >
                    {count}
                  </motion.span>
